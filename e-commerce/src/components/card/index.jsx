...
const Card = ({products}) => {
  ...
  return (
    <div  className='w-1/5 border-2 cursor-pointer p-4 rounded-lg gap-3 m-2 flex flex-col items-center text-center transition color hover:border-[#000]'>
        ...
        <button onClick={addCards} type='button' className='w-[130px]  p-2 relative flex items-center justify-center border-b-2 border-black transition-colors rounded-xl text-black hover:bg-black hover:text-white '>Add to cart <SiWearos  size={20} className='ml-2' /></button>
    </div>
  )
}
