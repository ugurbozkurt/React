...
const Card = ({products}) => {
  ...
  return (
    <div  className='w-1/5 border-2 cursor-pointer p-4 rounded-lg gap-3 m-2 flex flex-col items-center text-center transition color hover:border-[#000]'>
        ...
        <button onClick={addCards} type='button' className='w-[130px] bg-black text-[#FFF]  p-2 relative flex items-center justify-center border-b-2   border-black transition-colors rounded-xl  hover:border-b-2 hover:bg-white hover:text-black '>Add to cart <SiWearos  size={20} className='ml-2' /></button>
    </div>
  )
}
