...
export default function More() {
    return (
        ...
        <Popover className="relative">
            <Popover.Button className="h-14 block outline-none  rounded-full hover:bg-[#EFF3F41A]">
                <div className="p-3 rounded-full  transition-colors inline-flex items-center gap-5 group-hover:bg-[#EFF3F41A]">
                    <div className="w-[26.25] h-[26.25] relative">
                        <svg viewBox="0 0 24 24" width={26.25} height={26.25}>
                            <path fill="#text-[color:var(--background-font-primary)]" d="M3.75 12c0-4.56 3.69-8.25 8.25-8.25s8.25 3.69 8.25 8.25-3.69 8.25-8.25 8.25S3.75 16.56 3.75 12zM12 1.75C6.34 1.75 1.75 6.34 1.75 12S6.34 22.25 12 22.25 22.25 17.66 22.25 12 17.66 1.75 12 1.75zm-4.75 11.5c.69 0 1.25-.56 1.25-1.25s-.56-1.25-1.25-1.25S6 11.31 6 12s.56 1.25 1.25 1.25zm9.5 0c.69 0 1.25-.56 1.25-1.25s-.56-1.25-1.25-1.25-1.25.56-1.25 1.25.56 1.25 1.25 1.25zM13.25 12c0 .69-.56 1.25-1.25 1.25s-1.25-.56-1.25-1.25.56-1.25 1.25-1.25 1.25.56 1.25 1.25z" />
                        </svg>
                    </div>
            ...
            <Popover.Panel className="w-[318px] absolute bottom-0 left-0 bg-black shadow-box rounded-xl overflow-hidden">
                ...
                {MoreMenu.map((item, index) =>
                    <Disclosure key={index} >
                        {({ open }) => (
                            <>
                                <Disclosure.Button key={index} className="h-[52px] w-full flex items-center transition-colors justify-between px-4 font-bold hover:bg-[#EFF3F41A]">
                                    {item.mainmenu}
                                ...
                                <Disclosure.Panel className="relative" >
                                    {
                                        item.submenu.map((item, index) =>
                                            <Popover.Button onClick={item.func && (()=> setModal('appearance'))} type="button" key={index} className="w-full flex items-center px-3 h-11 gap-3 text-[15px] font-medium hover:bg-[#EFF3F41A]">
